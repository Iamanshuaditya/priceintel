...
test('PostgreSQL is the authority for observation idempotency under concurrent replay', async (t) => {
  ...
  const observation: PriceObservation = {
    id:'obs_db_1', workspaceId:'ws_db', productId:'prod_db', competitorListingId:'listing_db',
    fetchedAt:at, verifiedAt:at, currency:'USD', price:100, stockStatus:'UNKNOWN',
    sourceMethod:'JSON_LD', extractorVersion:'jsonld-v2', confidence:0.95, crawlRunId:'run_db_1',
  };

  const results = await Promise.all([
    persistObservationAndEffects(pool, observation),
    persistObservationAndEffects(pool, observation),
  ]);
