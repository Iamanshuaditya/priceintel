...
function markdown(runAt: string, summary: ReturnType<typeof summarize>, attempts: AttemptResult[]) {
  ...
  const lines = [
    ...
    `- Wrong prices when observation expected: **${summary.wrongExpectedObservationPrices}**`,
    `- Unsafe observations against abstain/unavailable/blocked truth: **${summary.unexpectedObservations}**`,
    `- False price observations (combined): **${summary.falsePriceObservations}**`,
