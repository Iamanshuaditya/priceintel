...
test('failure never advances last successful crawl or appends history', () => {
  ...
  store.recordSuccessfulObservation('a', { id:'o1', workspaceId:'a', productId:'p', competitorListingId:'l', fetchedAt:t1, verifiedAt:t1, currency:'USD', price:100, inStock:true, sourceMethod:'JSON_LD', extractorVersion:'jsonld-v1', confidence:0.95, crawlRunId:'r1' });
...
test('duplicate crawl/observation ingestion is idempotent', () => {
  ...
  const observation = { id:'o1', workspaceId:'a', productId:'p', competitorListingId:'l', fetchedAt:at, verifiedAt:at, currency:'USD', price:100, inStock:true, sourceMethod:'JSON_LD' as const, extractorVersion:'jsonld-v1', confidence:0.95, crawlRunId:'r1' };
